...
void ObjModel::draw()
{
	...
	for (ObjModel::ObjGroup* group : groups)
	{
		...
		ObjModel::MaterialInfo* material = materials[group->materialIndex];
...
void Texture::load()
{
	...
	stbi_uc* image = stbi_load(fileName.c_str(), &imageWidth, &imageHeight, &imageComponents, 0);

	if (image != nullptr)
	{
		std::cout << "Image size: " << imageWidth << "x" << imageHeight << ", " << imageComponents << " compontent(s)" << std::endl;
		...
		case 4:
			...
			break;
		default:
			std::cout << "WARNING: Unhandled image components amount" << std::endl;
			break;
		}

		std::cout << "Texture generate" << std::endl;
		glGenTextures(1, &id);
		std::cout << "Texture bind " << id << std::endl;
		glBindTexture(GL_TEXTURE_2D, id);

		std::cout << "Texture build" << std::endl;
		...
		std::cout << "Texture param" << std::endl;
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);

		std::cout << "Texture enable" << std::endl;
